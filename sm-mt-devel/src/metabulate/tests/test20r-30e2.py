...
if __name__ == "__main__":
    ...
    dc30e2_a = mtdebug.Criteria(classes=['Query'],methods=['_makeKey'],notes=['summary'],
                            targets=[mtutils.Flatfile(path=debug_path,
                                                      name='DO30r-30e2_a',
                                                      type=debug_type)])
    dc30e2_b = mtdebug.Criteria(methods=['_solveTriple','_solveTriple1'],
                            targets=[mtutils.Flatfile(path=debug_path,
                                                      name='DO30e2_b',
                                                      type=debug_type)])
    # set debug
...
#    d._update(criteria=[dc8f,dc12f,dc7f,dc13f,dc10f,dc14f,dc15f])
 ...
 #   d._update(criteria=[dc6,dc20f])
    ...
    s1._addNamespace('mytriples', 'http://www.semanticle.org/triples/')
    sb._addNamespace('mytriples', 'http://www.semanticle.org/triples/')
    sa._addNamespace('comtriples', 'http://www.semanticle.com/triples/')
    s1._addNamespace('comtriples', 'http://www.semanticle.com/triples/')
...
#    s1._actionTriple("add", ('ben', 'from', 'Truro'), 'child_of', 'alice')
    ...
    sb._actionTriple("add", 'eve', 'child_of', 'comtriples#dan')
    sa._actionTriple("add", 'eve1', 'child_of', 'http://www.semanticle.com/triples/#dan')
    s1._actionTriple("add", 'eve2', 'child_of', 'comtriples#dan')                                    # add fact to DTS (default _updateStore)
...
#    rlt04 = mtrules.Result(request=[[('?sub','?pred=ancestor3_of','?obj')]])                                               # pass
 ...
 #   rlt07 = mtrules.Result(request=[[('?sub','?pred','?obj')]])                                               # pass
 #   rlt07 = mtrules.Result(request=[[('?sub == ?','?pred','?obj')
 #                                     ,('?sub','?pred = =?','?obj')
 #                                     ,('?sub','?pred','?obj==?')]])                                            # pass - all rules
    rlt07 = mtrules.Result(request=[[('?sub == ?','?pred==*','?obj==*')
                                      ,('?sub==*','?pred = =?','?obj==*')
...
#                                     ,[('?person2', "ancestor3_of", 'eve')]])                                   # pass - syn of syn of ant of syn of recursed rule
    ...
    p5 = mtrender.Sequence(pattern=[({('np6','!p6'):{'?og':'?o'}},'is known by','?s')],
                           targets=['display'],
                           render='csv')
    p7 = mtrender.Sequence(pattern=[('?sub', '?pred', '?obj')],
                           targets=[s5,'display'],
                           render='csv',
                           URImode='nativealias')
...
#    rlt04._update(outputs=[p4,p0t,p0r,p0q])
    ...
    face07 = mtfacade.Facade(store=s1,
                             results=[rlt07])
...
#    print (s4._toString())
    ...
    print 'results instantiated'
    print (s5._toString())
    print 'should be 2 rule results'
    print
#    print ('contributory triples instantiated')
#    print (s3._toString())
#    print ('contributory rules instantiated')
