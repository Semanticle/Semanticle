...
#    d._update(criteria=[dc8f,dc12f,dc7f,dc13f,dc10f,dc14f,dc15f])
 ...
 #   d._update(criteria=[dc6,dc20f])
    ...
    f6 = mtutils.Flatfile(path=result_path,
                          ...
                          type=result_type)
    # stores
    s1 = mtstores.DistTripleStore()                                                     #  DTS s1
...
#    s1._actionTriple("add", ('ben', 'from', 'Truro'), 'child_of', 'alice')
    ...
    sb._actionTriple("add", (['eddyl','enderl'], 'from', 'London'), 'child_of', 'http://www.semanticle.com/triples/#dan')    # add fact to DTS._queryStore a
    ...
    sb._actionTriple("add", 'eve2', 'child_of', 'comtriples#dan')                                    # add fact to DTS (default _updateStore)
 ...
 #   sa._actionTriple("add","{('?desc1', 'desc_of', '?ancs'):[[[('?child', 'child_of', '?ancs')],[('?desc1', 'desc_of', '?child')]]]}")                   # add rule clause 2 to DTS._queryStore b (or change to DTS s1)
    sb._actionTriple("add","{('?desc1', 'desc_of', '?ancs'):[[[('?desc1', 'desc_of', '?child')],[('?child', 'child_of', '?ancs')]]]}")                   # add rule clause 2 to DTS._queryStore b (or change to DTS s1)
...
#                                     ,[('?person2', "ancestor3_of", 'eve')]])                                   # pass - syn of syn of ant of syn of recursed rule
    ...
    p5 = mtrender.Sequence(pattern=[({('np6','!p6'):{'?og':'?o'}},'is known by','?s')],
                           targets=['display'],
                           render='csv')
    p7 = mtrender.Sequence(pattern=[('?a', '?r', '?b')],
                           targets=[s5,'display'],
                           render='csv',
                           URImode='nativealias')
