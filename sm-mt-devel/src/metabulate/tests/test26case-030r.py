...
if __name__ == "__main__":
    ...
    dc26_020r = mtdebug.Criteria(methods=['_solve'],notes=['summary']
                                ,targets=[mtutils.Flatfile(path=debug_path
                                                          ,name='DebugOutput_dc26-020r'
                                                          ,type=debug_type)])
    # set debug
   # d = mtdebug.Debug()
...
#    d._update(criteria=[dc8f,dc12f,dc7f,dc13f,dc10f,dc14f,dc15f])
 ...
 #   d._update(criteria=[dc6,dc20f])
    ...
    f6 = mtutils.Flatfile(path=result_path,
                          ...
                          type=result_type)
    # stores
    sa = mtstores.TripleStore(structure='rbtree',tripleOrder=['p','s','o'])                                       #  TS sa rbtree
    sr = mtstores.TripleStore(structure='rbtree',tripleOrder=['p','s','o'])                                                         #  TS sr
    ...
    sa._actionTriple("add", [('child_of', 'cev', 'http://www.semanticle.org/triples/#bob'),"('child_of', 'http://www.semanticle.com/triples/#dan', 'cev')"])
