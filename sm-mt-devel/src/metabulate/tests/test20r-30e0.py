...
#    d._update(criteria=[dc8f,dc12f,dc7f,dc13f,dc10f,dc14f,dc15f])
 ...
 #   d._update(criteria=[dc6,dc20f])
    ...
    f6 = mtutils.Flatfile(path=result_path,
                          ...
                          type=result_type)
    # stores
    s1 = mtstores.TripleStore(unloadfile=fu)                                            # TS s1
    sr = mtstores.TripleStore(loadfile=fu)                                              # TS sr
...
#    s1._actionTriple("add", ('ben', 'from', 'Truro'), 'child_of', 'alice')
    ...
    s1._actionTriple("add", 'mytriples#bob', 'child_of', (['andrew','andy'], 'from', 'London'))
    s1._actionTriple("add", 'http://www.semanticle.org/triples/#bob', 'child_of', (['alex','arny'], 'from', 'Truro'))
    s1._actionTriple("add [('mytriples#bob', 'child_of', 'alice'),('http://www.semanticle.com/triples/#dan', 'child_of', 'cev')]")
    s1._actionTriple("add", [('cev', 'child_of', 'http://www.semanticle.org/triples/#bob'),"('http://www.semanticle.com/triples/#dan', 'child_of', 'cev')"])
    s1._actionTriple("add", 'eve', 'child_of', 'comtriples#dan')
...
#    s1._actionTriple("add","{('?desc1', 'desc_of', '?ancs'):[[[('?desc1', 'desc_of', '?child')],[('?child', 'child_of', '?ancs')]]]}")                   # add rule clause 2 to DTS._queryStore b (or change to DTS s1)
