...
#    s1._update(infer=False)
    ...
    sxp = mtstores.DistTripleStore(structure='dict',tripleOrder=['p','o','s'])                                         #  TS sa dict
    sxp._import(file=xp,mode='merge')
    print ('1. from parent store order file -> pos')
    print(sxp._toString())
    print
    sxs = mtstores.DistTripleStore(structure='dict',tripleOrder=['p','o','s'])                                         #  TS sa dict
    sxs._import(file=xs,mode='merge')
    print ('2. from sources store orders file -> pos')
    print(sxs._toString())
    print
    sxi = mtstores.DistTripleStore(structure='dict',tripleOrder=['p','o','s'])                                         #  TS sa dict
    sxi._import(file=xi,mode='merge')
    print ('3. from identified store order file -> pos')
    print(sxi._toString())
    print
    sxl = mtstores.DistTripleStore(structure='dict',tripleOrder=['p','o','s'])                                         #  TS sa dict
