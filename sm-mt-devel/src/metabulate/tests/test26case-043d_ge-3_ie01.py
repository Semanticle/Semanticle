...
#    s1._update(infer=False)
    ...
    sxp = mtstores.TripleStore(structure='dict',tripleOrder=['p','s','o'])                                         #  TS sa dict
    sxp._import(file=xp,mode='merge')
    print ('1. from parent store order file -> pso')
    print(sxp._toString())
    print
    sxs = mtstores.TripleStore(structure='dict',tripleOrder=['p','s','o'])                                         #  TS sa dict
    sxs._import(file=xs,mode='merge')
    print ('2. from sources store orders file -> pso')
    print(sxs._toString())
    print
    sxi = mtstores.TripleStore(structure='dict',tripleOrder=['p','s','o'])                                         #  TS sa dict
    sxi._import(file=xi,mode='merge')
    print ('3. from identified store order file -> pso')
    print(sxi._toString())
    print
    sxl = mtstores.TripleStore(structure='dict',tripleOrder=['p','s','o'])                                         #  TS sa dict
