...
#    d._update(criteria=[dc8f,dc12f,dc7f,dc13f,dc10f,dc14f,dc15f])
 ...
 #   d._update(criteria=[dc6,dc20f])
    ...
    f6 = mtutils.Flatfile(path=result_path,
                          ...
                          type=result_type)
    # stores
    s1 = mtstores.DistTripleStore(structure='rbtree',tripleOrder=['s','o','p'])                                         #  TS sa rbtree
#    sr = mtstores.DistTripleStore(structure='rbtree',tripleOrder=['s','o','p'])                                         #  TS sr rbtree
    sa = mtstores.TripleStore(structure='rbtree',tripleOrder=['s','o','p'])
    sb = mtstores.TripleStore(structure='rbtree',tripleOrder=['s','p','o'])
 #   sb = mtstores.TripleStore(structure='rbtree',tripleOrder=['s','o','p'])
...
#    s1._actionTriple("add", 'desc_of', 'syn_of', 'descr_of')                              # rev syn
 #   sa._actionTriple("add", 'anc_of', 'rev_of', 'des_of')                                 # ant of syn
  ...
  #  sb._actionTriple("add", 'des_of', 'anc_of', 'rev_of')                                 # ant of syn
