...
def gennoun(s1,l):
    a = 'a'*l
    z = 'n'*l
    for b in range(ord('a'),ord('n')+1):
        o1 = s1+chr(b)
        if l>2 and o1<=z[:l-2] and len(o1)<(l-1):
            for (s,o) in gennoun(o1,l): yield (s,o)
        s2 = o1+a[:(l-len(o1))]
        for c in range(ord('b'),ord('n')+1):
            o2 = o1+chr(c)
            if l>2 and o2<=z[:l-2] and len(o2)<(l-1):
                for (s,o) in gennoun(o2,l): yield (s,o)
            o3 = o2+a[:(l-len(o2))]
            yield (s2,o3)
...
#    d._update(criteria=[dc8f,dc12f,dc7f,dc13f,dc10f,dc14f,dc15f])
 ...
 #   d._update(criteria=[dc6,dc20f])
    ...
    # generate and instantiate interlinked triples
    for (subject,object) in gennoun('',2): sa._actionTriple('add', subject, 'child_of', object)
    # instantiate rules
