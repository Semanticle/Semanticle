...
if __name__ == "__main__":
    ...
    dc8f = mtdebug.Criteria(methods=['_actionTriple','_processTriple'],
                            targets=[mtutils.Flatfile(path=debug_path,
                                                      name='DebugOutput_dc4',
                                                      type=debug_type)])
    dc23f = mtdebug.Criteria(classes=['Transformation'],methods=['_generate'],
                            targets=[mtutils.Flatfile(path=debug_path,
                                                      name='DebugOutput_dc23',
                                                      type=debug_type)])
    # set debug
    d = mtdebug.Debug()
    # assign it the criteria
#    d._update(criteria=[dc8f,dc12f,dc7f,dc13f,dc10f,dc14f,dc15f])
    d._update(criteria=[dc23f])
    # stores
    s1 = mtstores.TripleStore()
    s2 = mtstores.TripleStore()
    # triples
    s1._actionTriple("add", 'fred', 'knows', 'bob')
    s1._actionTriple("add", ['fred','simone'], 'instance_of', 'person')
    s1._actionTriple("add", 'fido', 'instance_of', 'dog')
    s1._actionTriple("add", 'fred', 'has_job_title', 'manager')
    s1._actionTriple("add", 'fred', 'instance_of', 'person')
    s1._actionTriple("add", 'fred', 'instance_of', 'christian_name')
    s1._actionTriple("add", 'jill', 'says', [('fred', 'likes', 'simone'),
                                           ('simon', 'likes', 'simone'),
                                           ('fred', 'likes', 'fred'),
                                           ('fred', 'likes', 'fido')])
    s1._actionTriple("add", ('steven', 'from', 'London'), 'knows', ('fred', 'likes', 'janice'))
    s1._actionTriple("add", (['steven','simon'], 'from', 'London'), 'knows', ('fred', 'likes', 'simone'))
