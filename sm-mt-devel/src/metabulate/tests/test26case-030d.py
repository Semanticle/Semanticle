...
Related:
    ...
    DTS mixed triple ordering, import and export tests
        ...
        reversed order syn/antonyms dict   structure >= match
            ...
            test26case-043d_ge-3_ie02 - various export orders, re-import
                                      - store IO ordering tested for store input and facade querying
                                      - re-imported stores requeried exploiting facade IO ordering setting
...
if __name__ == "__main__":
    ...
    dc26_020d = mtdebug.Criteria(methods=['_solve']
                                ,targets=[mtutils.Flatfile(path=debug_path
                                                          ,name='DebugOutput_dc26-020dx_ok'
                                                          ,type=debug_type)])
    # set debug
   # d = mtdebug.Debug()
...
#    d._update(criteria=[dc8f,dc12f,dc7f,dc13f,dc10f,dc14f,dc15f])
 ...
 #   d._update(criteria=[dc6,dc20f])
    ...
    f6 = mtutils.Flatfile(path=result_path,
                          ...
                          type=result_type)
    # stores
    sa = mtstores.TripleStore(structure='dict',tripleOrder=['s','o','p'])                                         #  TS sa dict
    sr = mtstores.TripleStore(structure='dict',tripleOrder=['s','o','p'])                                         #  TS sr dict
    ...
    sa._actionTriple("add", [('cev', 'http://www.semanticle.org/triples/#bob', 'child_of'),"('http://www.semanticle.com/triples/#dan', 'cev', 'child_of')"])
...
#                           [[('?child', 'child_of', '?ancs')],[('?desc', 'desc_of', '?child')]]
 ...
 #   sa._actionTriple("add","{('?desc1', 'desc_of', '?ancs'):[[[('?child', 'child_of', '?ancs')],[('?desc1', 'desc_of', '?child')]]]}")                   # add rule clause 2 to DTS._queryStore b (or change to DTS s1)
    sa._actionTriple("add","{('?desc1', '?ancs', 'desc_of'):[[[('?desc1', '?child', 'desc_of')],[('?child', '?ancs', 'child_of')]]]}")                   # add rule clause 2 to DTS._queryStore b (or change to DTS s1)
...
#    s1._actionTriple("add", 'desc_of', 'syn_of', 'descr_of')                              # rev syn
    sa._actionTriple("add", 'anc_of', 'des_of', 'rev_of')                                 # ant of syn
  #  sa._actionTriple("add", 'des_of', 'rev_of', 'anc_of')                                 # ant of syn
 #   sa._actionTriple("add", 'ancs_of', 'rev_of', 'desc_of')                               # ant
#    s1._actionTriple("add", 'ancestor1_of', 'syn_of', 'ancs_of')                          # syn of ant
 #   sa._actionTriple("add", 'ancestor2_of', 'syn_of', 'ancs_of')                           # syn of ant of syn
